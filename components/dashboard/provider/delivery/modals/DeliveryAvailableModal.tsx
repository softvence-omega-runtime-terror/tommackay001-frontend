...
export default function DeliveryAvailableModal({ task, open, onClose }: Props) {
        ...
        <div className="flex flex-col max-h-[90vh]">
          <div className="px-6 pt-6 pb-4 border-b border-[#e9eaeb] shrink-0 ">
            ...
            <div className="flex justify-end mt-3">
              <span className="text-xs font-bold bg-[#ECFDF3] text-[#027A48] border border-[#6CE9A6] px-3.5 py-1 rounded-full tracking-widest">
                AVAILABLE
              </span>
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto  px-6 py-5 space-y-6">
            <section>
              ...
              <div className="border border-[#e9eaeb] rounded-xl mt-3 space-y-2 ">
                <div className="grid grid-cols-2 gap-x-8 px-5 pt-4">
                  <SpecField
                    label="Project Title"
                ...
                <div className="grid grid-cols-2 gap-x-8 px-5">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-1.5 mt-1 text-sm text-[#535862]">
                ...
                <div className="p-5">
                  ...
                  <div className="flex items-center gap-2 mt-2 bg-[#f2f4f7] rounded-lg px-3 py-2">
                    <Globe className="w-4 h-4 text-[#4169E1] shrink-0" />
                    <a
                      href="https://startuply.io.com"
                      className="text-sm text-[#535862] hover:text-[#4169E1] transition-colors truncate"
                    >
          ...
          <div className="shrink-0 px-6 pb-6 pt-4 flex justify-between border-t border-[#e9eaeb]  gap-3">
            <button
              onClick={onClose}
              className="flex-1 cursor-pointer bg-[#f2f4f7] max-w-100 hover:bg-[#e9eaeb] text-[#535862] font-semibold text-sm py-3.5 rounded-xl transition-colors tracking-wide"
            >
              CLOSE OVERSIGHT
            </button>
            <button
              onClick={() => handleSubmitProposal()}
              className="flex-2 cursor-pointer  bg-secondary max-w-100 hover:bg-[#d94118] text-white font-bold text-sm py-3.5 rounded-xl transition-colors flex items-center justify-center gap-2 tracking-wide"
            >
...
function SectionHeading({ children }: { children: React.ReactNode }) {
  return (
    <h3 className="text-xs font-bold uppercase tracking-widest text-[#181d27]">
      {children}
    </h3>
  );
}
...
function SpecLabel({ children }: { children: React.ReactNode }) {
  return (
    <p className="text-xs font-bold uppercase tracking-wide text-[#181d27]">
      {children}
    </p>
  );
}
...
function SpecField({ label, value }: { label: string; value: string }) {
  return (
    <div>
      ...
      <p className="text-sm text-[#535862] mt-1">{value}</p>
    </div>
  );
}
...
function StatCard({
  ...
  return (
    <div className="bg-[#f9fafb] rounded-xl p-4 flex flex-col items-center text-center gap-2">
      <span className="text-[#535862]">{icon}</span>
      <p className="text-[10px] font-bold uppercase tracking-wide text-[#535862] leading-snug">
        {label}
      </p>
      <p className="text-2xl font-bold text-[#181d27]">{value}</p>
    </div>
  );
}
